...
class Player:
    ...
    def serve(self, state) -> tuple:
        """生成发球动作，需返回 (shot_type, landing_pos, height)"""
        pass

    def generate_shot(self, state):
        """生成击球动作"""
        pass
...
class SamplePlayer:
    ...
    def generate_shot(self, state):
        ...
        if self.shot_count <= 2:
            return self._safe_shot(state)
        
        # 第三次开始按概率触发失败
        return self._random_failure_shot(state)
    
    def _safe_shot(self, state):
        """安全击球（类型1或5）"""
        shot_type = random.choice([1, 5])  # 长球/平球
        target = (4, 1) if state[-1] == 0 else (2, 1)  # 合法落点
        return (shot_type, target, 2)  # 高度2避免挂网

    def _random_failure_shot(self, state):
        ...
        if failure_type == 2:    # 挂网
            ...
            return (4, (6, 3), 2) if state[-1] == 0 else (4, (-1, 2), 2)
